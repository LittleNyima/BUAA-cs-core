...
free_block(u_int blockno)
{
	...
	bitmap[blockno >> 5] = bitmap[blockno >> 5] | (1 << (blockno % 32));
}
...
alloc_block(void)
{
	...
	if ((r = alloc_block_num()) < 0) { // failed.
		ULOG("alloc_block() find free block fail");
		return r;
	}
	bno = r;
