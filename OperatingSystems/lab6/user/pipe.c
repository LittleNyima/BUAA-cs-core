...
piperead(struct Fd *fd, void *vbuf, u_int n, u_int offset)
{
	...
	while (p->p_rpos >= p->p_wpos) {
		if (_pipeisclosed(fd, p)) {
			ULOG("pipe reader: pipe is closed, return 0");
			return 0;
		}
		syscall_yield();
	}
	...
	for (i = 0; i < n; i++) {
		while (p->p_rpos >= p->p_wpos) {
			if (_pipeisclosed(fd, p)) {
				ULOG("pipe reader: pipe is closed, return i");
				return i;
			}
			syscall_yield();
		}
	...
	ULOG("pipe reader: finish reading, return n");
	return n;
...
pipewrite(struct Fd *fd, const void *vbuf, u_int n, u_int offset)
{
	...
	for (i = 0; i < n; i++) {
		while (p->p_wpos - p->p_rpos >= BY2PIPE) {
			if (_pipeisclosed(fd, p)) {
				ULOG("pipe writer: pipe closed, return 0");
				return 0;
			}
			syscall_yield();
		}
