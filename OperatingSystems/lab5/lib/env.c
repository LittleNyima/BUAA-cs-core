...
env_setup_vm(struct Env *e)
{
	...
	e->env_pgdir[PDX(VPT)] = e->env_cr3 | PTE_V | PTE_R;
	LOG("env_setup_vm() return");
    return 0;
}
                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
	...
	u_long curva = ROUNDDOWN(va, BY2PG);

	LOG("load_icode_mapper() Step 1 start");
    /*Step 1: load all content of bin into memory. */
	if (offset > 0) {
		page_alloc(&p);
		page_insert(pgdir, p, curva, PTE_R|PTE_V);
		bcopy(bin, (void*)(page2kva(p) + offset), BY2PG - offset);
		i = BY2PG-offset;
	}
	LOG("load_icode_mapper() Step 1 bp 1");
	curva = ROUND(va, BY2PG);
	if (bin_size < BY2PG) {
		page_alloc(&p);
		page_insert(pgdir, p, curva, PTE_R|PTE_V);
    ...
    for (; bin_size > BY2PG && i < bin_size - BY2PG; i += BY2PG) {
		...
		printf("bin_size-BY2PG=%lu, i=%lu\n", bin_size-BY2PG, i);
    }
	LOG("load_icode_mapper() Step 1 bp 2");
	if (i < bin_size) {
		page_alloc(&p);
		page_insert(pgdir, p, curva, PTE_R|PTE_V);
		curva += BY2PG;
	...
	LOG("load_icode_mapper() Step 2 start");
    ...
    * hint: variable `i` has the value of `bin_size` now! */
    ...
    while (sgsize > BY2PG && i < sgsize - BY2PG) {
		page_alloc(&p);
		page_insert(pgdir, p, curva, PTE_R|PTE_V);
		curva += BY2PG;
		bzero((void*)page2kva(p), BY2PG);
